...
class Classifier(object):
  ...
  def __init__(self,num_feats,hidden_size,is_training=True):
    ...
    with graph.as_default():
      ...
      ### Adding variable for weight matrix for hidden layer 1
      w1 = tf.get_variable("w1",
                          [num_feats,self.hidden_size], # if you add a layer, change this
                          tf.float32)
      ### Adding variable for weight matrix for hidden layer 2
      w2 = tf.get_variable("w2",
                           [self.hidden_size, self.hidden_size],  # if you add a layer, change this
                           tf.float32)

      ### Adding variable for weight matrix for hidden layer 3
      # w3 = tf.get_variable("w3",
      #                      [self.hidden_size, self.hidden_size],  # if you add a layer, change this
      #                      tf.float32)

      ### Adding variable for weight matrix for outer layer
      u = tf.get_variable("u",
                           [self.hidden_size, 2],  # if you add a layer, change this
                           tf.float32)

      ### Adding bias for weight matrix for bias term for hidden layer 1
      b1 = tf.get_variable("b1",
                           [self.hidden_size],  # if you add a layer, change this
                           tf.float32)
      ### Adding bias for weight matrix for bias term for hidden layer 2
      b2 = tf.get_variable("b2",
                           [self.hidden_size],  # if you add a layer, change this
                           tf.float32)
      ### Adding bias for weight matrix for bias term for hidden layer 3
      # b3 = tf.get_variable("b3",
      #                      [self.hidden_size],  # if you add a layer, change this
      #                      tf.float32)

      ### Adding bias for weight matrix for bias term for outer layer
      bu = tf.get_variable("bu",
                           [2],  # if you add a layer, change this
                           tf.float32)

      ## Proabbility distribution
      h1 = tf.nn.xw_plus_b(self._feats, w1, b1)

      ###  Applying non linearity on layer 1
      g1 = tf.tanh(h1)

      ## Proabbility distribution
      h2 = tf.nn.xw_plus_b(g1, w2, b2)

      ###  Applying non linearity on layer 2
      g2 = tf.tanh(h2)

      # ## Proabbility distribution
      # h3 = tf.nn.xw_plus_b(g2, w3, b3)
      #
      # ###  Applying non linearity on layer 3
      # g3 = tf.tanh(h3)

      ## Proabbility distribution
      o = tf.nn.xw_plus_b(g2, u, bu)

      ###  Applying non linearity on outer layer
      otanh = tf.tanh(o)

      # Make it a probability distribution
      self._softmaxes = tf.nn.softmax(otanh)

      # Pick class
      self._pred_label = tf.argmax(self._softmaxes,1)

      # Loss calculated as to make the prob. dist. closer to target
      self._loss = loss = tf.nn.sparse_softmax_cross_entropy_with_logits(otanh,self._target_label)
...
def prediction_analysis(preds,targets,ids,reader):
  ...
  print("{:.2f}% of Adenoid tweets are positive".format(and_pct_pos))
  print("{:.2f}% of aPhone tweets are positive".format(iph_pct_pos))
